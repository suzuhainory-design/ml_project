...
class HRAttritionPredictorV2:
    ...
    def __init__(self, log_dir='../log', data_dir='../data', model_dir='../model'):
        ...
        self.logger = LogUtil(log_dir=log_dir, log_name=f'predict_v4_{timestamp}')
        self.logger.info("=" * 80)
        self.logger.info("V4版本HR员工流失预测开始")
        self.logger.info("=" * 80)
    ...
    def preprocess_test_data(self):
        ...
        # 处理类别特征（只处理最终特征中的类别特征）
        categorical_cols = self.test_df.select_dtypes(include=['object', 'category']).columns.tolist()
        # 只对最终特征中的类别特征进行编码
        categorical_cols_to_encode = [col for col in categorical_cols if col in self.feature_names and col in self.label_encoders]
        self.logger.info(f"需要编码的类别特征: {categorical_cols_to_encode}")
    ...
    def plot_results(self):
        ...
        plot_path = os.path.join(self.data_dir, 'prediction_results_v4.png')
    ...
    def save_predictions(self):
        ...
        output_path = os.path.join(self.data_dir, 'predictions_v4.csv')
